<?php
  /*
  Plugin Name: AWeber Forms by MailMunch
  Plugin URI: http://www.mailmunch.co
  Description: The AWeber plugin allows you to quickly and easily add signup forms for your AWeber lists. Popup, Embedded, Top Bar and a variety of different options available.
  Version: 1.0.5
  Author: MailMunch
  ...
  */
  ...
  define( 'AWEBER_MAILMUNCH_SLUG', "aweber-mailmunch");
  define( 'AWEBER_MAILMUNCH_VER', "1.0.5");
  define( 'AWEBER_MAILMUNCH_URL', "www.mailmunch.co");

  // Create unique WordPress instance ID
  if (get_option("aw_mm_wordpress_instance_id") == "") {
    update_option("aw_mm_wordpress_instance_id", uniqid());
  }

  // Adding Admin Menu
  add_action( 'admin_menu', 'aw_mm_register_page' );

  function aw_mm_register_page(){
    add_options_page('AWeber', 'AWeber', 'manage_options', AWEBER_MAILMUNCH_SLUG, 'aw_mm_setup');
    $menu_page = add_menu_page( 'AWeber Settings', 'AWeber', 'manage_options', AWEBER_MAILMUNCH_SLUG, 'aw_mm_setup', plugins_url( 'img/icon.png', __FILE__ ), 104.786 ); 
    // If successful, load admin assets only on that page.
    if ($menu_page) add_action('load-' . $menu_page, 'aw_mm_load_plugin_assets');
  }

  function aw_mm_plugin_settings_link($links) {
    $settings_link = '<a href="options-general.php?page='.AWEBER_MAILMUNCH_SLUG.'">Settings</a>';
    array_unshift($links, $settings_link);
    return $links;
  }

  $plugin = plugin_basename(__FILE__);
  add_filter('plugin_action_links_'.$plugin, 'aw_mm_plugin_settings_link');

  function aw_mm_load_plugin_assets() {
    add_action( 'admin_enqueue_scripts', 'aw_mm_enqueue_admin_styles' );
    add_action( 'admin_enqueue_scripts', 'aw_mm_enqueue_admin_scripts'  );
  }

  function aw_mm_enqueue_admin_styles() {
    wp_enqueue_style(AWEBER_MAILMUNCH_SLUG . '-admin-styles', plugins_url( 'css/admin.css', __FILE__ ), array(), AWEBER_MAILMUNCH_VER );
  }

  function aw_mm_enqueue_admin_scripts() {
    wp_enqueue_script(AWEBER_MAILMUNCH_SLUG . '-admin-script', plugins_url( 'js/admin.js', __FILE__ ), array( 'jquery' ), AWEBER_MAILMUNCH_VER );
  }

  // Adding MailMunch Asset Files (JS + CSS) 
  function aw_mm_load_asset_code() {
    $aw_mm_data = unserialize(get_option("aw_mm_data"));
    if (!$aw_mm_data["script_src"]) return;
    ...
    echo "(function(){ setTimeout(function(){ var d = document, f = d.getElementsByTagName('script')[0], s = d.createElement('script'); s.type = 'text/javascript'; s.async = true; s.src = '".$aw_mm_data["script_src"]."'; f.parentNode.insertBefore(s, f); }, 1); })();";
    echo "</script>";
  }

  add_action('init', 'aw_mm_assets');

  function aw_mm_assets() {
    $aw_mm_data = unserialize(get_option("aw_mm_data"));
    if (count($aw_mm_data) == 0) return;

    if (function_exists('wp_footer')) {
      add_action( 'wp_footer', 'aw_mm_load_asset_code' ); 
    }
    elseif (function_exists('wp_head')) {
      add_action( 'wp_head', 'aw_mm_load_asset_code' ); 
    }
  }

  function aw_mm_add_post_containers($content) {
    if (is_single() || is_page()) {
      $content = aw_mm_insert_form_after_paragraph("<div class='mailmunch-forms-in-post-middle' style='display: none !important;'></div>", "middle", $content);
  ...
  function aw_mm_register_sidebar_widget() {
      register_widget( 'AW_MM_Sidebar_Widget' );
  }
  add_action( 'widgets_init', 'aw_mm_register_sidebar_widget' );

  function aw_mm_insert_form_after_paragraph($insertion, $paragraph_id, $content) {
    $closing_p = '</p>';
  ...
  add_filter( 'the_content', 'aw_mm_add_post_containers' );

  function aw_mm_shortcode_form($atts) {
    return "<div class='mailmunch-forms-short-code mailmunch-forms-widget-".$atts['id']."' style='display: none !important;'></div>";
  }

  add_shortcode('mailmunch-form', 'aw_mm_shortcode_form');

  function aw_mm_setup() {
    $mm_helpers = new AweberMailmunchHelpers();
    $aw_mm_data = unserialize(get_option("aw_mm_data"));
    $aw_mm_data["site_url"] = home_url();
    $aw_mm_data["site_name"] = get_bloginfo();
    update_option("aw_mm_data", serialize($aw_mm_data));

    // This is a POST request. Let's save data first.
    if ($_POST) {
      $post_data = (isset($_POST["aw_mm_data"]) ? $_POST["aw_mm_data"] : array());
      $post_action = $_POST["action"];

      if ($post_action == "save_settings") { 

        $aw_mm_data = array_merge(unserialize(get_option('aw_mm_data')), $post_data);
        update_option("aw_mm_data", serialize($aw_mm_data));
      
      } else if ($post_action == "sign_in") {

        $mm = new AweberMailmunchApi($_POST["email"], $_POST["password"], "http://".AWEBER_MAILMUNCH_URL);
        if ($mm->validPassword()) {
          update_option("aw_mm_user_email", $_POST["email"]);
          update_option("aw_mm_user_password", base64_encode($_POST["password"]));
        ...
        if (empty($_POST["email"]) || empty($_POST["password"])) {
          ...
          $aw_mm_email = $account_info['email'];
          $aw_mm_password = $account_info['password'];

          $mm = new AweberMailmunchApi($aw_mm_email, $aw_mm_password, "http://".AWEBER_MAILMUNCH_URL);
          if ($mm->isNewUser($_POST['email'])) {
            $update_result = $mm->updateGuest($_POST['email'], $_POST['password']);
            $result = json_decode($update_result['body']);
            update_option("aw_mm_user_email", $result->email);
            update_option("aw_mm_user_password", base64_encode($_POST['password']));
            if (!$result->guest_user) { delete_option("aw_mm_guest_user"); }
            $aw_mm_email = $result->email;
            $aw_mm_password = $_POST['password'];

            // We have update the guest with real email address, let's create a site now
            $mm = new AweberMailmunchApi($aw_mm_email, $aw_mm_password, "http://".AWEBER_MAILMUNCH_URL);

            $update_result = $mm->updateSite($aw_mm_data["site_name"], $aw_mm_data["site_url"]);
            $result = json_decode($update_result['body']);
            $aw_mm_data = unserialize(get_option("aw_mm_data"));
            $aw_mm_data["site_url"] = $result->domain;
            $aw_mm_data["site_name"] = $result->name;
            update_option("aw_mm_data", serialize($aw_mm_data));
          } else {
            $user_exists = true;
          }
        ...
        if ($_POST["site_id"] && $_POST["widget_id"]) {
          ...
          $aw_mm_email = $account_info['email'];
          $aw_mm_password = $account_info['password'];
          $mm = new AweberMailmunchApi($account_info['email'], $account_info["password"], "http://".AWEBER_MAILMUNCH_URL);
        ...
        if (!empty($site->id)) {
          $aw_mm_data = unserialize(get_option("aw_mm_data"));
          $aw_mm_data["site_id"] = $site->id;
          $aw_mm_data["script_src"] = $site->javascript_url;
          update_option("aw_mm_data", serialize($aw_mm_data));
        }
      }
    }
    ...
    if (get_option("aw_mm_user_email") == "") {
      $aw_mm_email = "guest_".uniqid()."@mailmunch.co";
      $aw_mm_password = uniqid();
      $mm = new AweberMailmunchApi($aw_mm_email, $aw_mm_password, "http://".AWEBER_MAILMUNCH_URL);
      $mm->createGuestUser();
      update_option("aw_mm_user_email", $aw_mm_email);
      update_option("aw_mm_user_password", base64_encode($aw_mm_password));
      update_option("aw_mm_guest_user", true);
    }
    ...
    $aw_mm_guest_user = get_option("aw_mm_guest_user");
    ...
    if (isset($aw_mm_data["site_id"])) {
      // If there's a site already chosen, we need to get and save it's script_src in WordPress
      $site = $mm_helpers->getSite($sites, $aw_mm_data["site_id"]);
      
      if ($site) {
        $aw_mm_data = array_merge(unserialize(get_option('aw_mm_data')), array("script_src" => $site->javascript_url));
        update_option("aw_mm_data", serialize($aw_mm_data));
      } else {
        // The chosen site does not exist in the mailmunch account any more, remove it locally
        $site_not_found = true;
        $aw_mm_data = unserialize(get_option('aw_mm_data'));
        unset($aw_mm_data["site_id"]);
        unset($aw_mm_data["script_src"]);
        update_option("aw_mm_data", serialize($aw_mm_data));
      }
    }
    ...
    if (!isset($aw_mm_data["site_id"])) {
      ...
      if (sizeof($sites) == 1 && ($sites[0]->name == get_bloginfo() || $sites[0]->name == "WordPress")) {
        ...
        if ($site) {
          $aw_mm_data = array_merge(unserialize(get_option('aw_mm_data')), array("site_id" => $site->id, "script_src" => $site->javascript_url));
          update_option("aw_mm_data", serialize($aw_mm_data));
        }
    ...
    <div id="choose-site-form">
      <form action="" method="POST">
        <div class="form-group">
          ...
          <select name="aw_mm_data[site_id]">
...
<?php
    ...
    $request = $mm->getWidgetsHtml($aw_mm_data["site_id"]);
    $widgets = $request['body'];
    $widgets = str_replace("{{EMAIL}}", $aw_mm_email, $widgets);
    $widgets = str_replace("{{PASSWORD}}", $aw_mm_password, $widgets);
    echo $widgets;
...
<div id="signup-signin-box" style="display:none;">
  <div id="sign-up-form" class="<?php if (!$_POST || ($_POST["action"] != "sign_in" && $_POST["action"] != "unlink_account")) { ?> active<?php } ?>">
    <div class="form-container">
      <p style="margin-bottom: 0px;">To activate your AWeber forms, we will now create your account on MailMunch (<a onclick="showWhyAccount();" id="why-account-btn">Why?</a>).</p>

      <div id="why-account" class="alert alert-warning" style="display: none;">
        <h4>Why do I need a MailMunch account?</h4>

        <p>
          MailMunch is a not just a WordPress plugin but a standalone service. An account is required to identify your WordPress and serve your AWeber forms.
        </p>
      </div>
      ...
      <form action="" method="POST">
        ...
        <div class="form-group">
          ...
          <input type="text" placeholder="Site Name" name="site_name" value="<?php echo $aw_mm_data["site_name"] ?>" class="form-control">
        </div>

        <div class="form-group">
          <label>Wordpress URL</label>
          <input type="text" placeholder="Site URL" name="site_url" value="<?php echo $aw_mm_data["site_url"] ?>" class="form-control">
        </div>
  ...
  <div id="sign-in-form" class="<?php if ($_POST && ($_POST["action"] == "sign_in" || $_POST["action"] == "unlink_account")) { ?> active<?php } ?>">
    ...
    <p>Forgot your password? <a href="http://<?php echo AWEBER_MAILMUNCH_URL; ?>/users/password/new" target="_blank">Click here</a> to retrieve it.</p>
